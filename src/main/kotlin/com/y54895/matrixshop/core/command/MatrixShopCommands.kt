...
object MatrixShopCommands {
    ...
    private fun handleMain(sender: CommandSender, label: String, args: List<String>): Boolean {
        ...
        if (args.isEmpty()) {
            sendPlayerHelp(player, label)
            return true
        }
        val shopEntry = resolveBoundShopEntry(args[0])
        val shopRoute = shopEntry?.takeUnless { it.selection.bindings.registerStandalone }?.route
        val moduleRoute = ModuleBindings.resolveModule(args[0])?.takeIf(::exposedUnderMain)
        when {
    ...
    private fun handleSystem(player: Player, args: List<String>) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "system-shop", CommandUsageContext.modulePrefix(player, "system-shop", "/ms system"))
            return
        }
        when (args[0].lowercase()) {
            "open" -> {
        ...
        when (args[0].lowercase()) {
            ...
            "amount" -> {
                if (args.getOrNull(1)?.equals("add", true) != true) {
                    sendUsage(player, currentModuleUsage(player, "system-shop", "confirm amount add <number>"))
                    return
                }
    ...
    private fun handleCart(player: Player, args: List<String>, defaultShopId: String? = null) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "cart", CommandUsageContext.modulePrefix(player, "cart", "/cart"), defaultShopId)
            return
        }
        when (args[0].lowercase()) {
            ...
            "remove" -> {
                ...
                if (index == null) {
                    sendUsage(player, currentModuleUsage(player, "cart", "remove <slot>", defaultShopId))
                    return
                }
            ...
            "amount" -> {
                ...
                if (index == null || amount == null) {
                    sendUsage(player, currentModuleUsage(player, "cart", "amount <slot> <number>", defaultShopId))
                    return
                }
    ...
    private fun handleGlobalMarket(player: Player, args: List<String>, defaultShopId: String? = null) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "global-market", CommandUsageContext.modulePrefix(player, "global-market", "/market"), defaultShopId)
            return
        }
        when (args[0].lowercase()) {
    ...
    private fun handlePlayerShop(player: Player, args: List<String>, defaultShopId: String? = null) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "player-shop", CommandUsageContext.modulePrefix(player, "player-shop", "/playershop"), defaultShopId)
            return
        }
        when (args[0].lowercase()) {
            "open" -> {
            ...
            "upload" -> {
                ...
                val price = args.getOrNull(priceIndex)?.toDoubleOrNull()
                if (price == null) {
                    sendUsage(player, currentModuleUsage(player, "player-shop", "upload <price> [amount]", explicitShopId ?: defaultShopId))
                    return
                }
                val amount = args.getOrNull(priceIndex + 1)?.toIntOrNull()
    ...
    private fun handleRecord(player: Player, args: List<String>, defaultShopId: String? = null) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "record", CommandUsageContext.modulePrefix(player, "record", "/record"), defaultShopId)
            return
        }
        when (args[0].lowercase()) {
            "open" -> {
            ...
            "detail" -> {
                ...
                if (recordId.isNullOrBlank()) {
                    sendUsage(player, currentModuleUsage(player, "record", "detail <id>", defaultShopId))
                    return
                }
    ...
    private fun handleAuction(player: Player, args: List<String>, defaultShopId: String? = null) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "auction", CommandUsageContext.modulePrefix(player, "auction", "/auction"), defaultShopId)
            return
        }
        when (args[0].lowercase()) {
            ...
            "detail" -> {
                ...
                if (id.isNullOrBlank()) {
                    sendUsage(player, currentModuleUsage(player, "auction", "detail <id>", explicitShopId ?: defaultShopId))
                    return
                }
            ...
            "bid" -> {
                ...
                if (id.isNullOrBlank()) {
                    sendUsage(player, currentModuleUsage(player, "auction", "bid <id> [price]", explicitShopId ?: defaultShopId))
                    return
                }
            ...
            "buyout" -> {
                ...
                val id = args.getOrNull(if (explicitShopId != null) 2 else 1)
                if (id.isNullOrBlank()) {
                    sendUsage(player, currentModuleUsage(player, "auction", "buyout <id>", explicitShopId ?: defaultShopId))
                    return
                }
            ...
            "remove" -> {
                ...
                val id = args.getOrNull(if (explicitShopId != null) 2 else 1)
                if (id.isNullOrBlank()) {
                    sendUsage(player, currentModuleUsage(player, "auction", "remove <id>", explicitShopId ?: defaultShopId))
                    return
                }
    ...
    private fun handleTransaction(player: Player, args: List<String>, defaultShopId: String? = null) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "transaction", CommandUsageContext.modulePrefix(player, "transaction", "/trade"), defaultShopId)
            return
        }
        when (args[0].lowercase()) {
    ...
    private fun handleChestShop(player: Player, args: List<String>, defaultShopId: String? = null) {
        ...
        if (args[0].equals("help", true)) {
            sendModuleHelp(player, "chestshop", CommandUsageContext.modulePrefix(player, "chestshop", "/chestshop"))
            return
        }
        when (args[0].lowercase()) {
    ...
    private fun sendPlayerHelp(player: Player, rootLabel: String) {
        ...
        addHelp(lines, true, helpLine("/$rootLabel help", "@commands.help.desc.main-help"))
        addHelp(lines, showModuleHelp("system-shop") && Permissions.has(player, PermissionNodes.SYSTEMSHOP_USE), helpLine(helpModuleUsage(rootLabel, "system-shop", "open [category]"), "@commands.help.desc.system-open"))
        ...
        addHelp(lines, showModuleHelp("menu") && Permissions.has(player, PermissionNodes.MENU_USE), helpLine(helpModuleUsage(rootLabel, "menu", "open"), "@commands.help.desc.menu-open"))
        addBoundShopHelp(lines, showModuleHelp("auction") && Permissions.has(player, PermissionNodes.AUCTION_USE), rootLabel, shopHelpEntries("auction"), "open", "@commands.help.desc.auction-open", "open")
        addHelp(lines, showModuleHelp("auction") && Permissions.has(player, PermissionNodes.AUCTION_SELL), helpLine(helpModuleUsage(rootLabel, "auction", "upload <english|dutch> <start> [buyout|end] [duration]"), "@commands.help.desc.auction-upload"))
        addHelp(lines, showModuleHelp("auction") && Permissions.has(player, PermissionNodes.AUCTION_BID), helpLine(helpModuleUsage(rootLabel, "auction", "bid <id> [price]"), "@commands.help.desc.auction-bid"))
        addHelp(lines, showModuleHelp("auction") && Permissions.has(player, PermissionNodes.AUCTION_BUYOUT), helpLine(helpModuleUsage(rootLabel, "auction", "buyout <id>"), "@commands.help.desc.auction-buyout"))
        addHelp(lines, showModuleHelp("auction") && Permissions.has(player, PermissionNodes.AUCTION_MANAGE_OWN), helpLine(helpModuleUsage(rootLabel, "auction", "manage | bids"), "@commands.help.desc.auction-manage"))
        addBoundShopHelp(lines, showModuleHelp("player-shop") && Permissions.has(player, PermissionNodes.PLAYERSHOP_USE), rootLabel, shopHelpEntries("player-shop"), "open", "@commands.help.desc.player-shop-open-self", "open")
        addHelp(lines, showModuleHelp("player-shop") && Permissions.has(player, PermissionNodes.PLAYERSHOP_USE), helpLine(helpModuleUsage(rootLabel, "player-shop", "open [player]"), "@commands.help.desc.player-shop-open-other"))
        addHelp(lines, showModuleHelp("player-shop") && Permissions.has(player, PermissionNodes.PLAYERSHOP_MANAGE_OWN), helpLine(helpModuleUsage(rootLabel, "player-shop", "edit"), "@commands.help.desc.player-shop-edit"))
        addHelp(lines, showModuleHelp("player-shop") && Permissions.has(player, PermissionNodes.PLAYERSHOP_SELL), helpLine(helpModuleUsage(rootLabel, "player-shop", "upload <price> [amount]"), "@commands.help.desc.player-shop-upload"))
        addBoundShopHelp(lines, showModuleHelp("global-market") && Permissions.has(player, PermissionNodes.GLOBALMARKET_USE), rootLabel, shopHelpEntries("global-market"), "open", "@commands.help.desc.global-market-open", "open")
        addHelp(lines, showModuleHelp("global-market") && Permissions.has(player, PermissionNodes.GLOBALMARKET_SELL), helpLine(helpModuleUsage(rootLabel, "global-market", "upload <price> [amount]"), "@commands.help.desc.global-market-upload"))
        addHelp(lines, showModuleHelp("global-market") && Permissions.has(player, PermissionNodes.GLOBALMARKET_MANAGE_OWN), helpLine(helpModuleUsage(rootLabel, "global-market", "manage"), "@commands.help.desc.global-market-manage"))
        addHelp(lines, showModuleHelp("chestshop") && Permissions.has(player, PermissionNodes.CHESTSHOP_USE), helpLine(helpModuleUsage(rootLabel, "chestshop", "open"), bindingHelpKeyOrDefault("chestshop", "@commands.help.desc.chestshop-open")))
        addHelp(lines, showModuleHelp("chestshop") && Permissions.has(player, PermissionNodes.CHESTSHOP_CREATE), helpLine(helpModuleUsage(rootLabel, "chestshop", "create <buy|sell|dual> <price> [sell-price] [amount]"), "@commands.help.desc.chestshop-create"))
        addHelp(lines, showModuleHelp("chestshop") && Permissions.has(player, PermissionNodes.CHESTSHOP_USE), helpLine(helpModuleUsage(rootLabel, "chestshop", "stock | history"), "@commands.help.desc.chestshop-stock-history"))
        addHelp(lines, showModuleHelp("chestshop") && Permissions.has(player, PermissionNodes.CHESTSHOP_MANAGE_OWN), helpLine(helpModuleUsage(rootLabel, "chestshop", "edit | remove | price | amount | mode"), "@commands.help.desc.chestshop-manage"))
        addHelp(lines, showModuleHelp("cart") && Permissions.has(player, PermissionNodes.CART_USE), helpLine(helpModuleUsage(rootLabel, "cart", "open"), bindingHelpKeyOrDefault("cart", "@commands.help.desc.cart-open")))
        addHelp(lines, showModuleHelp("cart") && Permissions.has(player, PermissionNodes.CART_CHECKOUT), helpLine(helpModuleUsage(rootLabel, "cart", "checkout [valid_only]"), "@commands.help.desc.cart-checkout"))
        addHelp(lines, showModuleHelp("cart") && Permissions.has(player, PermissionNodes.CART_CHECKOUT), helpLine(helpModuleUsage(rootLabel, "cart", "checkout confirm [valid_only] | conflict"), "@commands.help.desc.cart-conflict"))
        addHelp(lines, showModuleHelp("cart") && Permissions.has(player, PermissionNodes.CART_CLEAR), helpLine(helpModuleUsage(rootLabel, "cart", "remove <slot> | remove_invalid | clear"), "@commands.help.desc.cart-manage"))
        addHelp(lines, showModuleHelp("cart") && Permissions.has(player, PermissionNodes.CART_USE), helpLine(helpModuleUsage(rootLabel, "cart", "amount <slot> <number>"), "@commands.help.desc.cart-amount"))
        addHelp(lines, showModuleHelp("record") && Permissions.has(player, PermissionNodes.RECORD_USE), helpLine(helpModuleUsage(rootLabel, "record", "open [keyword]"), bindingHelpKeyOrDefault("record", "@commands.help.desc.record-open")))
        addHelp(lines, showModuleHelp("record") && Permissions.has(player, PermissionNodes.RECORD_DETAIL_SELF), helpLine(helpModuleUsage(rootLabel, "record", "detail <id>"), "@commands.help.desc.record-detail"))
        addHelp(lines, showModuleHelp("record") && Permissions.has(player, PermissionNodes.RECORD_USE), helpLine(helpModuleUsage(rootLabel, "record", "filter [module|all]"), "@commands.help.desc.record-filter"))
        addHelp(lines, showModuleHelp("record") && Permissions.has(player, PermissionNodes.RECORD_STATS_SELF), helpLine(helpModuleUsage(rootLabel, "record", "income | expense | stats"), "@commands.help.desc.record-stats"))
        addBoundShopHelp(lines, showModuleHelp("transaction") && Permissions.has(player, PermissionNodes.TRANSACTION_USE), rootLabel, shopHelpEntries("transaction"), "open", "@commands.help.desc.transaction-open", "open")
        addHelp(lines, showModuleHelp("transaction") && Permissions.has(player, PermissionNodes.TRANSACTION_USE), helpLine(helpModuleUsage(rootLabel, "transaction", "request <player>"), "@commands.help.desc.transaction-request"))
        addHelp(lines, showModuleHelp("transaction") && Permissions.has(player, PermissionNodes.TRANSACTION_USE), helpLine(helpModuleUsage(rootLabel, "transaction", "accept [player] | ready | confirm | cancel | logs"), "@commands.help.desc.transaction-control"))
        addHelp(lines, showModuleHelp("transaction") && Permissions.has(player, PermissionNodes.TRANSACTION_USE), helpLine(helpModuleUsage(rootLabel, "transaction", "money <amount> | exp <amount>"), "@commands.help.desc.transaction-offer"))
        Texts.sendRaw(player, lines.joinToString("\n"))
    }

    private fun sendModuleHelp(player: Player, moduleId: String, commandRoot: String, shopId: String? = null) {
        val lines = mutableListOf(Texts.tr("@commands.help.player-title"))
        lines += configuredHelpLines(player, moduleId, commandRoot, shopId)
        when (moduleId) {
            "menu" -> addHelp(lines, Permissions.has(player, PermissionNodes.MENU_USE), helpLine("$commandRoot open", "@commands.help.desc.menu-open"))
            "auction" -> {
                addHelp(lines, Permissions.has(player, PermissionNodes.AUCTION_USE), helpLine("$commandRoot open", bindingHintKeyOrDefault("auction", "open", "@commands.help.desc.auction-open")))
                addHelp(lines, Permissions.has(player, PermissionNodes.AUCTION_SELL), helpLine("$commandRoot upload <english|dutch> <start> [buyout|end] [duration]", "@commands.help.desc.auction-upload"))
                addHelp(lines, Permissions.has(player, PermissionNodes.AUCTION_BID), helpLine("$commandRoot bid <id> [price]", "@commands.help.desc.auction-bid"))
                addHelp(lines, Permissions.has(player, PermissionNodes.AUCTION_BUYOUT), helpLine("$commandRoot buyout <id>", "@commands.help.desc.auction-buyout"))
                addHelp(lines, Permissions.has(player, PermissionNodes.AUCTION_MANAGE_OWN), helpLine("$commandRoot manage | bids", "@commands.help.desc.auction-manage"))
            }
            "player-shop" -> {
                addHelp(lines, Permissions.has(player, PermissionNodes.PLAYERSHOP_USE), helpLine("$commandRoot open", bindingHintKeyOrDefault("player-shop", "open", "@commands.help.desc.player-shop-open-self")))
                addHelp(lines, Permissions.has(player, PermissionNodes.PLAYERSHOP_USE), helpLine("$commandRoot open [player]", "@commands.help.desc.player-shop-open-other"))
                addHelp(lines, Permissions.has(player, PermissionNodes.PLAYERSHOP_MANAGE_OWN), helpLine("$commandRoot edit", "@commands.help.desc.player-shop-edit"))
                addHelp(lines, Permissions.has(player, PermissionNodes.PLAYERSHOP_SELL), helpLine("$commandRoot upload <price> [amount]", "@commands.help.desc.player-shop-upload"))
            }
            "global-market" -> {
                addHelp(lines, Permissions.has(player, PermissionNodes.GLOBALMARKET_USE), helpLine("$commandRoot open", bindingHintKeyOrDefault("global-market", "open", "@commands.help.desc.global-market-open")))
                addHelp(lines, Permissions.has(player, PermissionNodes.GLOBALMARKET_SELL), helpLine("$commandRoot upload <price> [amount]", "@commands.help.desc.global-market-upload"))
                addHelp(lines, Permissions.has(player, PermissionNodes.GLOBALMARKET_MANAGE_OWN), helpLine("$commandRoot manage", "@commands.help.desc.global-market-manage"))
            }
            "transaction" -> {
                addHelp(lines, Permissions.has(player, PermissionNodes.TRANSACTION_USE), helpLine("$commandRoot open", bindingHintKeyOrDefault("transaction", "open", "@commands.help.desc.transaction-open")))
                addHelp(lines, Permissions.has(player, PermissionNodes.TRANSACTION_USE), helpLine("$commandRoot request <player>", "@commands.help.desc.transaction-request"))
                addHelp(lines, Permissions.has(player, PermissionNodes.TRANSACTION_USE), helpLine("$commandRoot accept [player] | ready | confirm | cancel | logs", "@commands.help.desc.transaction-control"))
                addHelp(lines, Permissions.has(player, PermissionNodes.TRANSACTION_USE), helpLine("$commandRoot money <amount> | exp <amount>", "@commands.help.desc.transaction-offer"))
            }
            "chestshop" -> {
                addHelp(lines, Permissions.has(player, PermissionNodes.CHESTSHOP_USE), helpLine("$commandRoot open", bindingHelpKeyOrDefault("chestshop", "@commands.help.desc.chestshop-open")))
                addHelp(lines, Permissions.has(player, PermissionNodes.CHESTSHOP_CREATE), helpLine("$commandRoot create <buy|sell|dual> <price> [sell-price] [amount]", "@commands.help.desc.chestshop-create"))
                addHelp(lines, Permissions.has(player, PermissionNodes.CHESTSHOP_USE), helpLine("$commandRoot stock | history", "@commands.help.desc.chestshop-stock-history"))
                addHelp(lines, Permissions.has(player, PermissionNodes.CHESTSHOP_MANAGE_OWN), helpLine("$commandRoot edit | remove | price | amount | mode", "@commands.help.desc.chestshop-manage"))
            }
            "cart" -> {
                addHelp(lines, Permissions.has(player, PermissionNodes.CART_USE), helpLine("$commandRoot open", bindingHelpKeyOrDefault("cart", "@commands.help.desc.cart-open")))
                addHelp(lines, Permissions.has(player, PermissionNodes.CART_CHECKOUT), helpLine("$commandRoot checkout [valid_only]", "@commands.help.desc.cart-checkout"))
                addHelp(lines, Permissions.has(player, PermissionNodes.CART_CHECKOUT), helpLine("$commandRoot checkout confirm [valid_only] | conflict", "@commands.help.desc.cart-conflict"))
                addHelp(lines, Permissions.has(player, PermissionNodes.CART_CLEAR), helpLine("$commandRoot remove <slot> | remove_invalid | clear", "@commands.help.desc.cart-manage"))
                addHelp(lines, Permissions.has(player, PermissionNodes.CART_USE), helpLine("$commandRoot amount <slot> <number>", "@commands.help.desc.cart-amount"))
            }
            "record" -> {
                addHelp(lines, Permissions.has(player, PermissionNodes.RECORD_USE), helpLine("$commandRoot open [keyword]", bindingHelpKeyOrDefault("record", "@commands.help.desc.record-open")))
                addHelp(lines, Permissions.has(player, PermissionNodes.RECORD_DETAIL_SELF), helpLine("$commandRoot detail <id>", "@commands.help.desc.record-detail"))
                addHelp(lines, Permissions.has(player, PermissionNodes.RECORD_USE), helpLine("$commandRoot filter [module|all]", "@commands.help.desc.record-filter"))
                addHelp(lines, Permissions.has(player, PermissionNodes.RECORD_STATS_SELF), helpLine("$commandRoot income | expense | stats", "@commands.help.desc.record-stats"))
            }
        }
        Texts.sendRaw(player, lines.joinToString("\n"))
    }

    private fun configuredHelpLines(player: Player, moduleId: String, commandRoot: String, shopId: String? = null): List<String> {
        val binding = configuredHelpBinding(moduleId, commandRoot, shopId)
        val helpLines = binding?.helpLines ?: ModuleBindings.helpLines(moduleId)
        if (helpLines.isEmpty()) {
            return emptyList()
        }
        val placeholders = mapOf(
            "command" to commandRoot,
            "binding" to commandRoot.removePrefix("/"),
            "shop-id" to (shopId ?: ""),
            "shop" to (shopId ?: "")
        )
        return helpLines.map { Texts.apply(it, placeholders) }
    }

    private fun configuredHelpBinding(moduleId: String, commandRoot: String, shopId: String?): ModuleCommandBinding? {
        val byShopId = shopId?.let { id ->
            shopHelpEntries(moduleId).firstOrNull { it.id.equals(id, true) }?.bindings
        }
        if (byShopId != null) {
            return byShopId
        }
        val normalized = commandRoot.removePrefix("/").substringBefore(' ').lowercase()
        return shopHelpEntries(moduleId).firstOrNull { entry ->
            entry.bindings.keys.any { it.equals(normalized, true) }
        }?.bindings ?: shopHelpEntries(moduleId).firstOrNull()?.bindings
    }
    ...
    private fun handleAdminGoods(sender: CommandSender, args: List<String>) {
        if (!Permissions.require(sender, PermissionNodes.ADMIN_GOODS)) {
            return
        }
    ...
    private fun addBoundShopHelp(
        ...
        entries.forEach { entry ->
            ...
            val descriptionKey = entry.bindings.hintKeys[actionKey.lowercase()] ?: entry.bindings.helpKey ?: fallbackDescriptionKey
            val command = if (entry.bindings.registerStandalone) {
                "/$key $suffix".trim()
            } else {
                "/$rootLabel $key $suffix".trim()
            }
            lines += helpLine(command, descriptionKey)
        }
    }
    ...
    private fun helpModuleUsage(rootLabel: String, moduleId: String, suffix: String, shopId: String? = null): String {
        return "${helpModulePrefix(rootLabel, moduleId, shopId)} $suffix".trim()
    }
    ...
    private fun bindingHintKeyOrDefault(moduleId: String, hintKey: String, fallbackKey: String): String {
        return ModuleBindings.hintKey(moduleId, hintKey) ?: fallbackKey
    }
    ...
    private fun adminUsage(suffix: String): String {
        return "/matrixshopadmin $suffix".trim()
    }
    ...
    private fun defaultModulePrefix(moduleId: String, shopId: String? = null): String {
        ...
        standaloneBoundShopEntries(moduleId).firstOrNull()?.selection?.bindings?.keys?.firstOrNull()?.let {
            return "/$it"
        }
        return if (ModuleBindings.registerStandalone(moduleId)) {
    ...
    private fun helpModulePrefix(rootLabel: String, moduleId: String, shopId: String? = null): String {
        if (shopId != null) {
            shopHelpEntries(moduleId).firstOrNull { it.id.equals(shopId, true) }?.bindings?.keys?.firstOrNull()?.let {
                return "/$it"
            }
        ...
        return if (ModuleBindings.registerStandalone(moduleId)) {
            ...
            "/$rootLabel ${ModuleBindings.primary(moduleId)}"
        }
    }

    private fun exposedUnderMain(moduleId: String): Boolean {
        return !ModuleBindings.registerStandalone(moduleId) && standaloneBoundShopEntries(moduleId).isEmpty()
    }
    ...
    private fun completeMain(sender: CommandSender, label: String, args: List<String>): List<String> {
        ...
        if (args.size == 1) {
            ...
            if (
                Permissions.has(sender, PermissionNodes.ADMIN_RELOAD) ||
                Permissions.has(sender, PermissionNodes.ADMIN_STATUS) ||
                Permissions.has(sender, PermissionNodes.ADMIN_MODULE) ||
                Permissions.has(sender, PermissionNodes.ADMIN_GOODS)
            ) {
                suggestions += "admin"
            }
            listOf("system-shop", "menu", "player-shop", "global-market", "auction", "transaction", "chestshop", "cart", "record")
                .filter(::exposedUnderMain)
                .forEach { moduleId -> suggestions += ModuleBindings.keys(moduleId) }
            suggestions += boundShopEntries()
                .filterNot { it.selection.bindings.registerStandalone }
                .flatMap { it.selection.bindings.keys }
        ...
        val shopEntry = resolveBoundShopEntry(first)
        val shopRoute = shopEntry?.takeUnless { it.selection.bindings.registerStandalone }?.route
        val moduleRoute = ModuleBindings.resolveModule(first)?.takeIf(::exposedUnderMain)
        return when {
    ...
    private fun resolveBoundShopRoute(token: String): ShopBindingRoute? {
        return resolveBoundShopEntry(token)?.route
    }

    private fun resolveBoundShopEntry(token: String): BoundShopEntry? {
        val normalized = token.trim().lowercase()
        return boundShopEntries().firstOrNull { it.selection.bindings.keys.contains(normalized) }
    }
